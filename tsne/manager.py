...
class Manager(object):
    ...
    def tsne(self, args, encoder, sample_set):
        ...
        # visualize
        tsne = TSNE(n_components=2, verbose=1, perplexity=10.0, random_state=2021)
            ...
            for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(sampler):
                ...
                if steps == 0:
                    ...
                    for i, relation in enumerate(current_relations):
                        tsne_data += test_data[relation]
                        labels += [relation] * len(test_data[relation])
                        colors_per_class[relation] = colors[i]
                
                z = self.tsne(args, encoder, tsne_data)
                df = pd.DataFrame()
                tx = z[:,0]
                ty = z[:,1]
                ...
                for label in labels:
                    ...
                    plt.scatter(current_tx, current_ty, c=color, label=label, s=20)
                plt.title(f'({args.mode}) task {steps}', fontsize=20)
                plt.savefig(os.path.join(args.output_dir, f'{args.mode} task {steps}'))
                plt.close()
                plt.style.use('seaborn')
